...
public class Main {
	...
	public void deleteList(Scanner scan, PickController pick){
...
public class PickController {
	...
	public void deleteList(int ordinal){
		...
		for(RandomPick item : pickList) {
			if(item.getOrdinal() == ordinal-1) {  //객체비교는 오류난다 왜 그럴까?
				pickList.remove(item);
			}
		}		
	}
	
	public RandomPick pickPerson() {
		int pickNumber = (int)(Math.random()*pickList.size()-1+1);
		for(RandomPick item : pickList) {
